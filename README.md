...
1. **Backend**: API REST creada con Django y Django Rest Framework (DRF), utilizando Simple JWT para autenticación segura mediante tokens. Implementa un sistema de gestión de usuarios y roles similar a _Spatie_ en _Laravel_ a través del sistema de permisos integrado de Django (o django-role-permissions).
...
- En Django, se utiliza su propio motor de grupos y permisos que actúa casi igual que _Spatie_ en Laravel.
...
- **App Router:** `src/app/`. Aquí residen las páginas (login, dashboard, eventos).
...
- Al acceder al `/dashboard` o `/eventos`, se valida el token localmente y hacia el servidor si se requieren datos.
...
- `python manage.py startapp <nombre_app>`: Crea nuevas apps como usuarios, eventos o tickets.
