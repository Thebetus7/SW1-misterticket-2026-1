...
import { Users, Plus, Trash2, X, ShieldAlert, User, Mail, Lock, UserCheck } from 'lucide-react';
...
export default function VendedoresPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const vendedoresData = await fetchApi('/usuarios/vendedores/');
      setVendedores(vendedoresData || []);

      if (isAdmin) {
        const promotoresData = await fetchApi('/usuarios/promotores/');
        // django rest framework router devuelve la lista directo
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Si es administrador, debemos enviar el promotor seleccionado
      if (isAdmin) {
        if (!formData.promotor) {
          throw new Error("Debe seleccionar un promotor para este vendedor.");
        }
        payload.promotor = parseInt(formData.promotor);
      }

      await fetchApi('/usuarios/vendedores/', {
        method: 'POST',
  ...
  const handleDelete = async (id) => {
    if (!confirm('¿Seguro que deseas eliminar este vendedor? Esto suspenderá su cuenta y perfil (soft delete).')) return;
    try {
      await fetchApi(`/usuarios/vendedores/${id}/`, {
        method: 'DELETE'
      });
      loadData();
    } catch (err) {
  ...
  const toggleVendedorEstado = async (vendedor) => {
    const nuevoEstado = vendedor.estado === 'activo' ? 'inactivo' : 'activo';
    try {
      await fetchApi(`/usuarios/vendedores/${vendedor.id}/`, {
        method: 'PATCH',
        ...
        <div className="max-w-7xl mx-auto space-y-8">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-brand-100 animate-fade-in-up">
            <div>
              ...
              <p className="text-brand-600 mt-1">
                {isAdmin 
                  ? 'Gestiona y supervisa a todos los vendedores de la plataforma.' 
                  : 'Crea y administra tus vendedores autorizados para escanear tickets.'
                }
              </p>
            </div>
          ...
          {loading ? (
            ...
            <div className="text-center p-16 bg-white rounded-2xl border border-brand-100 shadow-sm animate-fade-in-up">
              ...
              <h3 className="text-xl font-bold text-brand-700">No hay vendedores</h3>
              <p className="text-brand-500 mt-2">Aún no has agregado vendedores a tu equipo. ¡Comienza a expandir tu equipo ahora!</p>
            </div>
          ) : (
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-brand-200">
                  <thead className="bg-brand-50/50">
                    <tr>
                      <th className="px-6 py-4 text-left text-xs font-bold text-brand-500 uppercase tracking-wider">Vendedor</th>
                  ...
                  <tbody className="bg-white divide-y divide-brand-100">
                    {vendedores.map((vend) => (
                      <tr key={vend.id} className="hover:bg-brand-50/30 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="flex items-center gap-3">
                            <div className="w-9 h-9 rounded-full bg-accent/10 text-accent flex items-center justify-center font-bold text-sm">
                              {vend.usuario_first_name?.substring(0, 1) || vend.usuario_username?.substring(0, 1) || 'V'}
                            </div>
                            <div>
                              <div className="text-sm font-bold text-brand-900">
                                {vend.usuario_first_name ? `${vend.usuario_first_name} ${vend.usuario_last_name || ''}`.trim() : 'Sin nombre'}
                              </div>
                              <div className="text-xs text-brand-500">@{vend.usuario_username}</div>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-brand-600">
                          {vend.usuario_email || 'Sin correo'}
                        </td>
                        {isAdmin && (
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-brand-700 font-medium">
                            {vend.promotor_razon || 'No asignado'}
                          </td>
                        )}
                        <td className="px-6 py-4 whitespace-nowrap">
                          <button 
                            onClick={() => toggleVendedorEstado(vend)}
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <input 
                      ...
                      name="first_name" 
                      required 
                      className="input-mt py-2 text-sm" 
                      placeholder="Juan" 
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      name="username" 
                      required 
                      className="input-mt pl-9 py-2 text-sm" 
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input 
                      ...
                      required 
                      className="input-mt pl-9 py-2 text-sm" 
                      placeholder="juan@vendedores.com" 
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <select 
                        name="promotor" 
                        required 
                        className="input-mt py-2.5 text-sm" 
                ...
                <div className="pt-4 border-t border-brand-100 flex justify-end gap-2">
                  <button 
                    type="button" 
                    onClick={closeModal} 
                    className="px-4 py-2 rounded-xl font-semibold text-brand-600 hover:bg-brand-100 transition-colors text-sm"
                  >
                    Cancelar
                  </button>
                  <button 
                    type="submit" 
                    className="btn-ticket py-2 px-6 shadow-md shadow-accent/10 text-sm"
                  >
                    Crear Vendedor
                  </button>
                </div>
